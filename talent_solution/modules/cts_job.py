...
from google.cloud.talent_v4beta1.types import FieldMask as Job_Mask
...
class Job():
                        ...
                        for batch_id in batch_info:
                            if 'operation' in batch_info[batch_id]:
                                ...
                                if (batch_op.done() and not batch_info[batch_id]['done']):
                                    ...
                                    logger.debug("Waiting on batch {}".format(batch_id))
                                    print("Waiting on batch {}".format(batch_id))
                            else:
    ...
    def update_job(self,project_id,tenant_id=None,input_job=None,path=None,update_mask=None):
        logger.debug("logger:CALLED: create_job({},{},{},{})".format(project_id,tenant_id,input_job,path))
        try:
            client = self.client()
            #Prepare Request Metadata
            job_req_metadata = config.APP['request_metadata']           
            logger.debug("Setting request Metadata to {}".format(job_req_metadata))
            if update_mask:
                update_FieldMask=Job_Mask(paths=update_mask.split(","))
                logger.info("Fields to be updated in jobs input: {}".format(update_FieldMask))
                print("Fields to be updated in jobs input: {}".format(update_FieldMask))
            else:
                logger.info("Update Mask is not provided. For best experience, an update mask should be provided so only the fields in the mask are updated.")
                print("Update Mask is not provided. For best experience, an update mask should be provided so only the fields in the mask are updated.")

            def update_job_task(file):
                if not isinstance(file,str):
                    raise ValueError("Invalid parameter input to update job task.")
                
                file_name = os.path.basename(file)
                logger.debug("Batching the file {} @ {} rows in {} concurrent batches to be posted".format(file_name,batch_size,conc_batches))
                for batch in cts_helper.generate_file_batch(file=file,rows=batch_size,concurrent_batches=conc_batches):
                    try:
                        for batch_id,jobs in batch.items():
                            batch_key = file_name + "_" + str(batch_id)
                            batch_info.update({batch_key:{"batch":{},"operation":None,"posted":None,"done":False,"error":None}})
                            # Check each job in the batch exists already and remove it from the batch
                            parsed_jobs = collections.OrderedDict()
                            for line,job in jobs.items():
                                batch_info[batch_key]['batch'].update({line:{}})
                                try:
                                    job_json = json.loads(job)
                                    company_id = job_json['company']
                                    external_id = job_json['requisition_id']
                                    language=job_json['language_code']
                                    batch_info[batch_key]['batch'].update({line:{"company":company_id,"requisition_id":external_id,\
                                        "language_code":language,"status":"READ","errors":[]}})
                                except (AttributeError,json.JSONDecodeError) as e:
                                    batch_info[batch_key]['batch'].update({line:{'status':'ERROR','errors':[]}})
                                ...
                                # Parse the job if it isn't a malformed job. Some of the identifying fields and the fields 
                                ...
                                if parsed_job and 'ERROR' not in parsed_job[0]:
                                    ...
                                    logger.info("Job {}:{} for company {} parsed.".format(external_id,\
                                        language,company_id))
                                    batch_info[batch_key]['batch'][line].update({'status':'PARSED'})
                                else:
                                    logger.warning("Line {}: Parse job failed for job {}:{} for company {}: {}".format(line, external_id,\
                                        language,company_id,parsed_job[0]))
                                    print("Line {}: Parse job failed for job {}:{} for company {}: {}".format(line,external_id,\
                                        language,company_id,parsed_job[0]))                                            
                                    batch_info[batch_key]['batch'][line].update({'status':'PARSE_FAILED','errors':[]})
                                    batch_info[batch_key]['batch'][line]['errors'].append(parsed_job[0])
                            if parsed_jobs:
                                # Getting ready to post the batch: get the parent
                                parent = cts_helper.get_parent(project_id,tenant_id)
                                logger.debug("Batch {}: Parent is set to {}.".format(batch_key,parent))
                                logger.debug("UPDATE: Batch {}: Posting {} jobs between lines {} to {}.".format(batch_key,len(parsed_jobs),\
                                    list(batch_info[batch_key]['batch'].keys())[0],list(batch_info[batch_key]['batch'].keys())[-1]))
                                print("UPDATE: Batch {}: Posting {} jobs between lines {} to {}".format(batch_key,len(parsed_jobs),\
                                    list(batch_info[batch_key]['batch'].keys())[0],list(batch_info[batch_key]['batch'].keys())[-1]))
                                # Post the jobs
                                batch_info[batch_key]['operation']= client.batch_update_jobs(parent=parent,jobs=parsed_jobs.values(),update_mask=update_FieldMask, metadata=[job_req_metadata])
                                batch_info[batch_key]['posted'] = parsed_jobs.keys()
                                for posted_line in batch_info[batch_key]['posted']:
                                    batch_info[batch_key]['batch'][posted_line]['status'] = 'POSTED'
                            else:
                                logger.warning("No jobs to post in batch {}".format(batch_key))
                                print("No jobs to post in batch {}".format(batch_key))
                                batch_info[batch_key]['done']=True
                    except KeyError as e:
                        logger.error("Invalid file format found at: {}".format(file))
                        continue
                    except RetryError as e:
                        batch_info[batch_key]['done'] = True
                        batch_info[batch_key]['error']= "API Retry failed due to {}.".format(e)
                        logger.error("Batch {}: API Retry failed due to {}.".format(batch_key,e),\
                            exc_info=config.LOGGING['traceback'])
                    except GoogleAPICallError as e:
                        batch_info[batch_key]['error']="API Retry failed due to {}.".format(e)
                        logger.error("Batch {}: CTS API Error due to {}".format(batch_key,e),\
                            exc_info=config.LOGGING['traceback'])
            ...
            if path is None:
                try:
                    ...
                    if existing_job is None:
                        print("Job {}:{} for company {} does not exist.".format(external_id,language,company_id))
                        logger.debug("Job {}:{} for company {} does not exist.".format(external_id,language,company_id))
                    else:
                        logger.debug("update_job: Parsing job string: {}".format(input_job))
                        parsed_job = cts_helper.parse_job(project_id=project_id,tenant_id=tenant_id,jobs=[input_job])[0]
                        if parsed_job is None:
                            raise UnparseableJobError
                        else:
                            parsed_job['name'] = existing_job[0].name
                        parent = cts_helper.get_parent(project_id,tenant_id)
                        logger.debug("create_job: Parent is set to {}".format(parent))
                        logger.debug("Posting parsed job {}".format(parsed_job))
                        updated_job = client.update_job(job=parsed_job,update_mask=update_FieldMask, metadata=[job_req_metadata])
                        if updated_job:
                            # TODO: update expiration to DB
                            # if cts_db.persist_to_db(object=updated_job,project_id=project_id,tenant_id=tenant_id,company_id=company_id):
                            return updated_job
                        else:
                            raise Exception("Updating job {}:{} for company {} failed at the server.".format(external_id,language,company_id))
                        
                except UnparseableJobError as e:
                    logger.error("Unable to parse the input job string.",exc_info=config.LOGGING['traceback'])
                except ValueError:
                    logger.error("Invalid Parameters",exc_info=config.LOGGING['traceback'])
                    return False
                except GoogleAPICallError as e:
                    logger.error("API error when creating job. Message: {}".format(e),exc_info=config.LOGGING['traceback'])
                    return False
                except Exception as e:
                    logger.error("Error when creating job. Message: {}".format(e),exc_info=config.LOGGING['traceback'])
                    return False

            # Update a batch of jobs. Job strings are provided in a new line delimited JSON file as input.
            else:                
                if os.path.exists(path):
                    logger.debug("Reading input from {}".format(path))
                    batch_size = config.BATCH_PROCESS['batch_size'] or 200
                    conc_batches = config.BATCH_PROCESS['concurrent_batches'] or 1
                    batch_info = {}
                    ...
                    with concurrent.futures.ThreadPoolExecutor(max_workers = config.BATCH_PROCESS['concurrent_batches']) as executor:
                        for file in files:
                            executor.submit(update_job_task,file)

                    # Check if all batches are done
                    all_done = False
                    while not all_done:    
                        for batch_id in batch_info:
                            if batch_info[batch_id]['operation']:
                                batch_op = batch_info[batch_id]['operation']
                                if (batch_op.done() and not batch_info[batch_id]['done']):
                                    batch_result = batch_op.result()
                                    logger.debug("Batch ID {} results:\n".format(batch_id))
                                    for posted_line,result in zip(batch_info[batch_id]['posted'],batch_result.job_results):
                                        ...
                                        # Handle job creation failures
                                            ...
                                            batch_info[batch_id]['batch'][posted_line]['status'] = 'FAILED'
                                            batch_info[batch_id]['batch'][posted_line]['errors'].append(result.status)
                                            logger.info("Failed to update job {}:{} for company {}."\
                                            .format(batch_info[batch_id]['batch'][posted_line]['requisition_id'],\
                                                batch_info[batch_id]['batch'][posted_line]['language_code'],\
                                                    batch_info[batch_id]['batch'][posted_line]['company']))
                                            print("Failed to update job {}:{} for company {}."\
                                            .format(batch_info[batch_id]['batch'][posted_line]['requisition_id'],\
                                                batch_info[batch_id]['batch'][posted_line]['language_code'],\
                                                    batch_info[batch_id]['batch'][posted_line]['company']))
                                    batch_info[batch_id]['done']=True
                    ...
                    total_jobs_failed = 0
                    for batch_id,batch in batch_info.items():
                        print ("\nIn file shard: {}".format(batch_id))
                        for line,line_item in batch['batch'].items():
                            total_jobs_updated+=1 if line_item['status']=='SUCCESS' else 0
                            total_jobs_skipped+=1 if line_item['status']=='SKIPPED' else 0
                            total_jobs_failed+=1 if line_item['status']=='FAILED' else 0
                            ...
                            if line_item['status'] != 'SUCCESS':
                                print("Line {}:\nJob {}:{} for company {} {}\nErrors: {}".format(line, \
                                    line_item['requisition_id'],line_item['language_code'],line_item['company'],\
                                        line_item['status'],line_item['errors']))
                    print("Total Jobs updated: {}".format(total_jobs_updated))
                    print("Total Jobs skipped: {}".format(total_jobs_skipped))
                    print("Total Jobs failed: {}".format(total_jobs_failed))
                    #Check all the batches for errors: batch_errors = {batch_id:errors[]}
                    return True
            ...
            if input_job:
                ...
                logger.error("Error creating jobs from path {}. Message: {}".format(path,e),exc_info=config.LOGGING['traceback'])
            return False
        ...
        try:
            ...
            if all:
                ...
                if external_id:
                    ...
                    if scope=='full':
                        print("Listing all jobs for tenant {} and company {}. This could take a while...".format(tenant_id,company_id))
                ...
                # TODO: filter out the expired jobs
                job_key = job_key + '%'
